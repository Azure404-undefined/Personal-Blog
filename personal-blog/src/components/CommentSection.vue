...
const visibleReplies = (root: API.Comments.TreeComment) => {
  return expandedThreads.value.has(root._id)
    ? root.replies
    : root.replies.slice(0, REPLIES_PREVIEW);
};
...
const handleSubmitReply = async (rootComment: API.Comments.TreeComment) => {
  ...
  try {
    ...
    ElMessage({ message: e?.message || '回复失败', type: 'error', duration: 3000 });
  } finally {
...
const handlePostRoot = async () => {
  ...
  try {
    ...
    ElMessage({ message: e?.message || '评论失败', type: 'error', duration: 3000 });
  } finally {
...
const handleDelete = async (id: string) => {
  ...
  try {
    ...
    ElMessage({ message: e?.message || '删除失败', type: 'error', duration: 3000 });
  }
};
